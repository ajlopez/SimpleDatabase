...
function Database() {
    ...
    this.tableList = function () {
        return new RunRef(function () {
            return new Cursor(Object.keys(tables));
        });
    };
}
...
function GetUpdateRef(fn, key, values) {
    this.run = function () {
        ...
        for (var n in values) {
            ...
            if (typeof value === 'object' && value.evaluate && typeof value.evaluate === 'function')
...
function AddExpression(expr, value) {
    this.evaluate = function (document) {
        return expr.evaluate(document) + value;
    }
}
